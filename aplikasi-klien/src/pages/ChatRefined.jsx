...
const ChatRefined = () => {
  ...
  const [selectedModel, setSelectedModel] = useState(() => {
    // Load from localStorage or use default (4 models: Llama, DeepSeek, Gemini Pro, GPT-4 Turbo)
    return localStorage.getItem('selectedLLMModel') || 'meta-llama/llama-3.3-70b-instruct';
  }); // Selected LLM model
  ...
  useEffect(() => {
    ...
    Object.keys(pendingMessages).forEach(chatId => {
      ...
      if (allPendingInContext && pending.length > 0) {
        console.log('Γ£à [CHAT-REFINED] All pending messages now in context, clearing pending for chat:', chatId);
  ...
  useEffect(() => {
    if (activeChatId && contextChats[activeChatId]) {
      ...
      if (import.meta.env.DEV) {
        console.log('≡ƒôÑ [CHAT-REFINED] Loading messages for display:', {
          activeChatId,
      ...
      if (import.meta.env.DEV) {
        console.log('≡ƒôª [CHAT-REFINED] Loading merged messages:', {
          activeChatId,
      ...
      if (hasMessages) {
        if (import.meta.env.DEV) {
          console.log('≡ƒùæ∩╕Å [CHAT-REFINED] Clearing messages - no active chat');
        }
        clearMessages();
      }
    }
  ...
  //         
  //       } else {
  //         console.error('Γ¥î [CHAT-REFINED] Failed to save new chat:', result.error);
  //       }
  //     });
  //   }
  ...
  useEffect(() => {
    ...
    const handleJiraStateChanged = (event) => {
      if (event.detail?.action === 'connectionDeleted') {
        ...
        // setTimeout(() => {
        //   console.log(`≡ƒöä [CHAT-REFINED] Secondary UI refresh after JIRA state change`);
        //   setSidebarForceUpdate(prev => prev + 1);
        //   setActiveProjectUpdateTrigger(prev => prev + 1);
        // }, 100);
        ...
        // setTimeout(() => {
        //   console.log(`≡ƒöä [CHAT-REFINED] Final UI refresh after JIRA state change`);
        //   setSidebarForceUpdate(prev => prev + 1);
        //   setActiveProjectUpdateTrigger(prev => prev + 1);
        // }, 500);
      }
  ...
  // ✅ Handle switch model event from LimitExceededCard
  useEffect(() => {
    const handleSwitchModel = (event) => {
      const { modelName } = event.detail;
      
      console.log('🔄 [CHAT-REFINED] Switching model to:', modelName);
  ...
  useEffect(() => {
    const handleRetryAfterCooldown = (event) => {
  ...
  const handleSendMessage = async (text) => {
    console.log('≡ƒÜÇ [CHAT-REFINED] handleSendMessage called:', {
    ...
    if (!currentId) {
      console.log('≡ƒåò [CHAT-REFINED] Creating new chat...');
      ...
      if (result.success) {
        ...
        console.log('Γ£à [CHAT-REFINED] New chat created:', currentId);
    ...
    sendMessage(text, { 
      ...
      onMessageReceived: async (message, chatId) => {
        ...
        await updateChatMessages(chatId, (currentMessages) => {
          console.log('≡ƒôè [CHAT-REFINED] Current messages in update function:', {
            chatId,
          ...
          if (messageExists) {
            console.log('ΓÜá∩╕Å [CHAT-REFINED] Message already exists, skipping:', message.id);
        ...
        console.log('Γ£à [CHAT-REFINED] Message saved to context');
        ...
        if (message.role === 'ai') {
          setPendingMessages(prev => {
            const updated = { ...prev };
            delete updated[chatId];
            console.log('≡ƒº╣ [CHAT-REFINED] Cleared pending messages for chat:', chatId);
            return updated;
          });
        }
      }
  ...
  const handleSelectChat = (id) => {
    ...
    console.log('≡ƒöä [CHAT-REFINED] Selecting chat:', {
      chatId,
    ...
    console.log('≡ƒôª [CHAT-REFINED] Merging messages for chat:', {
      chatId,
    ...
    console.log('Γ£à [CHAT-REFINED] Merged messages:', {
      chatId,
  ...
  const handleUpdateMessage = async (updatedMessage) => {
    try {
      ...
      if (!activeChatId) {
        console.error('Γ¥î [CHAT-REFINED] No active chat ID for message update');
        return;
      }
      ...
      if (updatedMessage.currentVersionIndex !== undefined && !updatedMessage.isRegenerating) {
        console.log('≡ƒöä [CHAT-REFINED] Version navigation:', {
      ...
      if (isUserMessageChanged) {
        console.log('Γ£Å∩╕Å [CHAT-REFINED] User message edited, regenerating AI response:', {
        ...
        setTimeout(() => {
          ...
          console.log('≡ƒñû [CHAT-REFINED] Sending edited message to AI for new response');
      ...
      console.error('Γ¥î [CHAT-REFINED] Failed to update message:', error);
  ...
  const handleForceEpicClear = async () => {
    ...
    try {
      ...
      console.log('≡ƒÜ½ [CHAT-REFINED] Epic context blocked with ENHANCED protection:');
      ...
      console.error('Γ¥î [CHAT-REFINED] Error during force clear:', error);
    }
  };
  ...
  const handleRenameChat = async (chatId, newTitle) => {
    ...
    try {
      console.log('Γ£Å∩╕Å [CHAT-REFINED] Renaming chat:', chatId, 'to:', newTitle.trim());
      ...
      if (result.success) {
        ...
        console.error('Γ¥î [CHAT-REFINED] Failed to rename chat:', result.error);
      }
    } catch (error) {
      console.error('Γ¥î [CHAT-REFINED] Error renaming chat:', error);
    } finally {
  ...
  const handleDeleteChat = (chatId) => {
    console.log('≡ƒùæ∩╕Å [CHAT-REFINED] handleDeleteChat called with chatId:', chatId);
    console.log('≡ƒùæ∩╕Å [CHAT-REFINED] contextHistory:', contextHistory);
    
    if (!contextHistory || !Array.isArray(contextHistory)) {
      console.error('Γ¥î [CHAT-REFINED] contextHistory is not an array:', contextHistory);
      return;
    }
    
    const chat = contextHistory.find(h => h.id.toString() === chatId.toString());
    console.log('≡ƒùæ∩╕Å [CHAT-REFINED] Found chat:', chat);
  ...
  const confirmDeleteChat = async () => {
    ...
    try {
      ...
      console.log('≡ƒùæ∩╕Å [CHAT-REFINED] Deleting chat:', chatIdStr);
      ...
      if (result.success) {
        ...
        console.error('Γ¥î [CHAT-REFINED] Failed to delete chat:', result.error);
      }
    } catch (error) {
      console.error('Γ¥î [CHAT-REFINED] Error deleting chat:', error);
    } finally {
  ...
  const selectedProjectKey = useMemo(() => {
    ...
    try {
      ...
      if (!activeProjectId && connections && connections.length > 0) {
        ...
        // FIXED: Throttle this log too
        if (now - lastLogTime > 2000) {
          console.log(`≡ƒöº [EPIC-MODAL] Auto-setting first connection as active:`, {
        ...
        setTimeout(() => {
          setActiveProjectDebounced(chatId, firstConnection.id, projectData)
            ...
            .catch(error => {
              console.error('Γ¥î [CHAT-REFINED] Error saving active project:', error);
  ...
  const allMessages = useMemo(() => {
    ...
    console.log('≡ƒöì [CHAT-REFINED] allMessages calculation:', {
      activeChatId,
        ...
        <div className="px-6 py-4 bg-transparent flex items-center h-[73px]">
          <div className="flex items-center justify-between w-full">
            ...
            <div className="flex items-center gap-3">
              ...
              {/* Testing Scenario Button - Only show when panel is closed */}
              {!isTestingScenarioPanelOpen && (
                <button
                  ...
                  onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#120C18'}
                  onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
                  title="Open Testing Scenario"
                >
                  {/* Testing Icon - Only icon, no text */}
                  <svg 
              ...
              <div className="w-full max-w-4xl mb-10">
                <div className="backdrop-blur-lg rounded-2xl border p-6 shadow-2xl" style={{ backgroundColor: '#09090A', borderColor: 'rgba(255, 255, 255, 0.05)' }}>
                  <textarea
                    ...
                    onFocus={() => {
                      console.log('≡ƒÄ» [INPUT-FOCUS] Welcome screen input focused:', {
                        activeChatId,
                        loadingChats,
            ...
            <div className="max-w-4xl mx-auto px-6 py-3">
              <div className="backdrop-blur-lg rounded-2xl border p-5 shadow-2xl" style={{ backgroundColor: '#09090A', borderColor: 'rgba(255, 255, 255, 0.05)' }}>
                <textarea
                  ...
                  onFocus={() => {
                    console.log('≡ƒÄ» [INPUT-FOCUS] Bottom input focused:', {
                      activeChatId,
                      loadingChats,
                      isDisabled: loadingChats[activeChatId],
                      allLoadingChats: JSON.stringify(loadingChats)
      ...
      <ErrorBoundary fallbackMessage="Failed to load dual evaluation testing modal">
        <DualTestingModal
          ...
          onSubmitTest={async (results) => {
            ...
            if (!results) {
              console.error('Γ¥î [CHAT-REFINED] Test results are null or undefined');
              return;
            }
            
            console.log('Γ£à [CHAT-REFINED] Dual evaluation completed:', results);
            ...
            if (selectedScenario && selectedScenarioIndex !== null) {
              ...
              console.log('≡ƒöì [CHAT-REFINED] Test completion data:', {
                messageId,
                chatId,
              ...
              if (!chatId || !messageId) {
                console.error('Γ¥î [CHAT-REFINED] chatId or messageId is missing from selectedScenario:', selectedScenario);
                return;
              }
              ...
              console.log('≡ƒÆ╛ [CHAT-REFINED] Saving combined dual result:', combinedResult);
              ...
              console.error('Γ¥î [CHAT-REFINED] Cannot save test results - missing scenario or index:', {
