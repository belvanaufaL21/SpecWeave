            ...
            {!isMobile && (
              <div className={`relative w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                isUser 
                  ? 'border' 
                  : isError 
                    ? (message.errorCode === 'USAGE_LIMIT_EXCEEDED' ? 'border' : 'bg-gradient-to-br from-red-500 to-red-600')
                    : 'border'
              }`}
              style={isUser ? { backgroundColor: '#160D14', borderColor: '#44273D' } : (!isError || message.errorCode === 'USAGE_LIMIT_EXCEEDED' ? { backgroundColor: '#120C18', borderColor: '#2C1A43' } : {})}
              >
                {isUser ? (
                  <span className="text-sm font-semibold" style={{ color: '#FF7AD0' }}>{getUserInitial()}</span>
                ) : (isError && message.errorCode !== 'USAGE_LIMIT_EXCEEDED') ? (
